...
// 자바 프로그램은 JVM(Java Virtual Machine)에 의해 실행됩니다. JVM은 프로그램 실행을 시작할 때, 특정 클래스의 main 메서드를 찾아서 실행합니다. 이 시점에는 아직 클래스의 인스턴스가 생성되지 않았습니다. 
// 따라서, main 메서드는 클래스의 인스턴스를 만들지 않고도 호출될 수 있어야 합니다. 이를 가능하게 하려면, main 메서드는 static이어야 합니다.
// 프로그램이 시작될 때(main 함수가 호출된 순간)는 아직 어떤 객체도 만들어지지 못합니다. 따라서, JVM이 main 메서드를 호출한 시점에는 클래스의 인스턴스가 없는 상태입니다. 

// 자바에서 main 메서드는 프로그램의 진입점이며, 이 메서드가 호출되기 전에는 객체를 생성할 수 있는 환경(메모리 공간)이 마련되지 않기 때문에 객체 생성이 불가능합니다. 이는 Java 프로그램의 안정적인 실행을 위한 설계상의 제약입니다.
...
// 목표?--> main 메서드의 일관된 정의! // 일반적인 경우?--> main 메서드가 호출되기 전에 객체를 생성할수 없다.// 특수한 경우?--> main 메서드가 호출되기 전에 객체를 생성할수 있다. 일반적인 경우와 특수한 경우 모두에서 일관되게 main 메서드를 정의하려면, static이 유일한 방법이다!